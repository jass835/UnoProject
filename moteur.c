...
{
    // Récupérer les informations sur la carte jouée et la dernière carte
    char couleur_carte_jouee = carte_jouee[0];
    char fonction_carte_jouee = carte_jouee[1];
    char couleur_derniere_carte = derniere_carte[0];
    char fonction_derniere_carte = derniere_carte[1];

    char couleur_carte_noir = derniere_carte[3];

    // Vérifier si la carte jouée est une carte noire (+4 ou joker)
    if (couleur_carte_jouee == 'K')
    {
        return true; // Les cartes noires peuvent toujours être jouées
    }

    if (couleur_derniere_carte == 'K' && couleur_carte_noir == couleur_carte_jouee)
    {
        return true;
    }

    // Vérifier si la couleur ou la fonction de la carte jouée correspond à la dernière carte
    if (couleur_carte_jouee == couleur_derniere_carte || fonction_carte_jouee == fonction_derniere_carte)
    {
        return true; // La carte peut être jouée selon les règles du Uno
    }
